...
public interface IRebitManualSave
{
    ...
    Task<RebitActionResult> SavePayloadForResubmit(string payload, string functionName, string? fileName = default, IDictionary<string, string>? destinationFileTags = default, Encoding? encoding = default);
    ...
    /// <returns>A task that represents the asynchronous save operation. The task result contains a RebitActionResult indicating
    /// the outcome of the operation.</returns>
    Task<RebitActionResult> SavePayloadForResubmit(Stream payload, string functionName, string? fileName = default, IDictionary<string, string>? destinationFileTags = default, Encoding? encoding = default);
}
